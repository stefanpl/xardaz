...
/**
 * Process a list of values in parallel through an async executor function.
 *
 ...
 *
 */
export async function processInParallel<V, R>(
  values: Array<V>,
  executor: (value: V) => Promise<R>
): Promise<Array<R>> {
  return processInBatches(values, executor, values.length);
}
